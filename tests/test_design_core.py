# filepath: /workspaces/SeqNado/tests/test_design_core_additional.py
import pytest
...
def test_assay_enum_roundtrip_and_lookup():
    # Ensure converting from value to enum works and roundtrips
    for member in Assay:
        looked_up = Assay(member.value)
        assert looked_up is member
    # Ensure membership by value works for a known value
    assert Assay("rna") == Assay.RNA
    assert Assay("chip") == Assay.CHIP


def test_pileup_and_scale_enums_have_expected_members():
    pileup_names = {m.value for m in PileupMethod}
    assert {"deeptools", "homer", "bamnado"} <= pileup_names

    scale_names = {m.value for m in ScaleMethod}
    assert {"unscaled", "csaw", "cpm", "rpkm", "spikein", "merged"} <= scale_names


def test_metadata_invalid_types_raise_validation_error():
    # Passing wrong types into Metadata should raise ValidationError
    with pytest.raises(ValidationError):
        Metadata(norm_group=123)  # norm_group should be a string

    with pytest.raises(ValidationError):
        Metadata(spikein_ratio="not-a-number")  # expecting a numeric or None


def test_metadata_equality_and_copy_independence():
    m1 = Metadata(norm_group="g1", consensus_group="c1", deseq2="d1")
    m2 = Metadata(norm_group="g1", consensus_group="c1", deseq2="d1")
    assert m1 == m2
    # model_copy should produce an equivalent but distinct object
    m3 = m1.model_copy()
    assert m3 == m1 and m3 is not m1


@pytest.mark.parametrize(
    "input_name,expected",
    [
        ("simple_sample_R1", "simple_sample"),
        ("no_change", "no_change"),
        ("sample_S1_L001_R1_001", "sample"),
        ("", ""),
    ],
)
def test_clean_sample_name_additional(input_name, expected):
...
def test_extract_read_number_additional(filename, expected):
...
def test_is_control_sample_variants(name, expected):
    assert is_control_sample(name) == expected
