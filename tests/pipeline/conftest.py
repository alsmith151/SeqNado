...
def ensure_all_genome_data(genome_path: Path):
    """
    Ensure all genome data files required for all assays are present before any tests run.
    ...
    # Extract STAR index if not already extracted
    ...
    if star_tar.exists() and not star_dir.exists():
        with tarfile.open(star_tar) as tar:
            tar.extractall(path=genome_path)
        # Flatten nested STAR directory if present
        nested = star_dir / star_dir.name
        if nested.exists() and nested.is_dir():
            for f in nested.iterdir():
                f.rename(star_dir / f.name)
            try:
                nested.rmdir()
            except Exception as e:
                print(f"[ERROR] Could not remove nested directory {nested}: {e}")

    # Extract Bowtie2 index if not already extracted
