...
dir_tests_run = os.path.join(dir_tests, "pipeline_atac")
...
def set_up():
    ...
    for fq in glob.glob(f"{dir_data_test}/atac*.fastq.gz"):
        shutil.copy(fq, ".")
    ...
    replacements_dict = {
        ...
        "MACS_OPTIONS": "-g 46709983 --nomodel --shift 147",
    }

    with open(f"{dir_repo}/config_atac.yml", "r") as r:
...
def test_pipeline():
    ...
    cmd = "ngs-pipeline atac make --local -p 4"
