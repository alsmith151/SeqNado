...
tryCatch({
  cat("=== DESeq2 Spike-in Normalisation ===\n")
  ...
  if (length(spikein_gene_names) == 0 || all(spikein_gene_names == "")) {
    cat("  No spike-in genes provided - using median ratio method on all genes\n")
    dds <- estimateSizeFactors(dds)
  } else {
    cat("  Spike-in genes requested:", paste(spikein_gene_names, collapse = ", "), "\n")
    spikein_genes <- which(rownames(counts) %in% spikein_gene_names)
    cat("  Spike-in genes found in data:", length(spikein_genes), "\n")
    if (length(spikein_genes) == 0) {
      cat("  WARNING: No spike-in genes found in count matrix - using median ratio method on all genes\n")
      dds <- estimateSizeFactors(dds)
    } else {
  ...
  for (i in seq_along(size_factors)) {
    cat("  ", names(size_factors)[i], ":", round(size_factors[i], 4), "\n")
  }
