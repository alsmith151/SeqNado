...
def parse_options():
    ...
    parser.add_argument(
        "to_remap_bam",
        help="input BAM file containing "
    ...
    parser.add_argument(
        "remap_bam",
        help="input BAM file containing " "remapped reads (with flipped alleles)",
    )
    parser.add_argument(
        "keep_bam", help="output BAM file to write " "filtered set of reads to"
    )
...
def filter_reads(remap_bam):
    ...
    for read in remap_bam:
        ...
        if len(words) < 4:
            raise ValueError(
                "expected read names to be formatted "
        ...
        if "-" in coord_str:
            ...
            # correct location
            if read.pos < read.next_reference_start or (
                read.pos == read.next_reference_start
                and read.is_read1
                and not read.is_read2
            ):
            ...
            if read.is_paired:
                ...
                if read.qname in read_pair_cache:
                    ...
                    if len(cigs) == 1:
                        ...
                        if (
                            (r1.cigarstring != r2.cigarstring)
                            and (r1.cigarstring in cigs)
                            and (r2.cigarstring in cigs)
                        ):
