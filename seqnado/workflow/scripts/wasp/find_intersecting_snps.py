...
class DataFiles(object):
    ...
    def __init__(
        ...
        self.keep_bam = pysam.Samfile(self.keep_filename, "w", template=self.input_bam)
        self.remap_bam = pysam.Samfile(
            self.remap_filename, "w", template=self.input_bam
        )
    ...
    def write(self, file_handle):
        ...
        if total > 0:
            ...
            if mismatch_pct > 10.0:
                sys.stderr.write(
                    "WARNING: many read SNP overlaps do not match "
...
def parse_options():
    parser = argparse.ArgumentParser(
        description="Looks for SNPs and indels "
    ...
    parser.add_argument(
        ...
        dest="is_paired_end",
        default=False,
        help=("Indicates that reads are paired-end " "(default is single)."),
    )
    ...
    parser.add_argument(
        ...
        default=False,
        help=(
            "Indicates that the input BAM file"
    ...
    parser.add_argument(
        ...
        "combinations than MAX_SEQs are discarded" % MAX_SEQS_DEFAULT,
    )

    parser.add_argument(
        "--max_snps",
        type=int,
        default=MAX_SNPS_DEFAULT,
        ...
        "usage (default=%d)." % MAX_SNPS_DEFAULT,
    )
    ...
    parser.add_argument(
        "bam_filename",
        action="store",
        help="Coordinate-sorted input BAM file " "containing mapped reads.",
    )
    ...
    if options.samples and not options.haplotype:
        ...
        sys.stderr.write(
            "WARNING: ignoring --samples argument "
...
def get_unique_haplotypes(haplotypes, phasing, snp_idx):
    ...
    h = np.ascontiguousarray(haps).view(
        np.dtype((np.void, haps.dtype.itemsize * haps.shape[1]))
    )
...
def generate_haplo_reads(
    read_seq, snp_idx, read_pos, ref_alleles, alt_alleles, haplo_tab, phase_tab
):
    """
...
def generate_reads(read_seq, read_pos, ref_alleles, alt_alleles):
    ...
    for i in range(len(read_pos)):
        ...
        for j in range(len(reads)):
            ...
            reads.append(read[:idx] + ref_alleles[i].decode("utf-8") + read[idx + 1 :])
            reads.append(read[:idx] + alt_alleles[i].decode("utf-8") + read[idx + 1 :])
    return set(reads)
...
def write_pair_fastq(fastq_file1, fastq_file2, orig_read1, orig_read2, new_pairs):
    ...
    for pair in new_pairs:
        ...
        pos_str = "%d-%d" % (
            min(orig_read1.pos + 1, orig_read2.pos + 1),
...
def filter_reads(
    files, max_seqs=MAX_SEQS_DEFAULT, max_snps=MAX_SNPS_DEFAULT, samples=None
):
    cur_chrom = None
    cur_tid = None
    seen_chrom = set([])
    ...
    for read in files.input_bam:
        ...
        if (cur_tid is None) or (read.tid != cur_tid):
            ...
            if len(read_pair_cache) != 0:
                sys.stderr.write(
                    "WARNING: failed to find pairs for %d "
            ...
            if cur_chrom in seen_chrom:
                ...
                raise ValueError(
                    "expected input BAM file to be sorted "
        ...
        if read.is_paired:
            if read.mate_is_unmapped:
                ...
                read.next_reference_name == cur_chrom or read.next_reference_name == "="
            ):
                ...
                if read.qname in read_pair_cache:
                    ...
                    if read2.next_reference_start != read1.reference_start:
                        sys.stderr.write(
                            "WARNING: read pair positions "
                            "do not match for pair %s\n" % read.qname
                        )
                    else:
                        process_paired_read(
                            read1, read2, read_stats, files, snp_tab, max_seqs, max_snps
                        )
                else:
            ...
            process_single_read(read, read_stats, files, snp_tab, max_seqs, max_snps)
...
def read_pair_combos(old_reads, new_reads, max_seqs, snp_idx, snp_read_pos):
    ...
    if slice_read(old_reads[0], snp_read_pos[0]) != slice_read(
        old_reads[1], snp_read_pos[1]
    ):
        return None
    # group reads by the alleles they have at shared SNPs
    for i in range(len(new_reads)):
        new_reads[i] = group_reads_by_snps(new_reads[i], snp_read_pos[i])
    unique_pairs = set()
...
def process_paired_read(read1, read2, read_stats, files, snp_tab, max_seqs, max_snps):
    ...
    for read in (read1, read2):
        ...
        snp_idx, snp_read_pos, indel_idx, indel_read_pos = snp_tab.get_overlapping_snps(
            read
        )
        ...
        if len(snp_idx) > 0:
            ...
            count_ref_alt_matches(read, read_stats, snp_tab, snp_idx, snp_read_pos)
            ...
            if files.hap_h5:
                ...
                read_seqs = generate_haplo_reads(
                    read.query_sequence,
                    snp_idx,
                    snp_read_pos,
                    ref_alleles,
                ...
                read_seqs = generate_reads(
                    read.query_sequence, snp_read_pos, ref_alleles, alt_alleles
                )
...
def process_single_read(read, read_stats, files, snp_tab, max_seqs, max_snps):
...
def parse_samples(samples_str):
    ...
    if os.path.exists(samples_str) and not os.path.isdir(samples_str):
        ...
        sys.stderr.write(
            "read %d sample names from file '%s'\n" % (len(samples), samples_str)
        )
...
def main(
    ...
    filter_reads(files, max_seqs=max_seqs, max_snps=max_snps, samples=samples)
