...
def update_read_cache(cur_by_mpos, keep_cache, discard_cache, read_stats, outfile):
    for mpos, read_list in list(cur_by_mpos.items()):
        ...
        if keep_read.qname in keep_cache:
            raise ValueError("read %s is already " "in keep cache" % keep_read.qname)
...
def filter_reads(infile, outfile):
    ...
    for read in infile:
        ...
        if (cur_tid is None) or (read.tid != cur_tid):
            ...
            if cur_pos:
                update_read_cache(
                    cur_by_mpos, keep_cache, discard_cache, read_stats, outfile
                )
        ...
        if (cur_pos is not None) and (read.pos < cur_pos):
            raise ValueError(
                "expected input BAM file to be sorted " "but reads are out of order"
            )
    ...
    if cur_pos:
        update_read_cache(cur_by_mpos, keep_cache, discard_cache, read_stats, outfile)
...
if __name__ == "__main__":
    ...
    parser.add_argument("output_bam", help="output BAM or SAM file (not " "sorted!)")
