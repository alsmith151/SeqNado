...
def create_carray(h5f, chrom, data_type):
    ...
    shape = [chrom.length]
    carray = h5f.create_carray(h5f.root, chrom.name, atom, shape, filters=zlib_filter)
...
def get_sam_iter(samfile, chrom):
    try:
        sam_iter = samfile.fetch(reference=chrom.name, start=1, end=chrom.length)
        ...
        sys.stderr.write(
            "WARNING: %s does not exist in BAM file, "
            "trying %s instead\n" % (chrom.name, chrom_name)
        )

        try:
            sam_iter = samfile.fetch(reference=chrom_name, start=1, end=chrom.length)
        except ValueError:
    ...
    if hap_tab:
        ...
        for i, read_offset in zip(snp_idx, read_offsets):
            ...
            if ind_idx * 2 > hap_tab.shape[1]:
                raise ValueError(
                    "index of individual (%d) is >= number of "
...
def add_read_count(
    ...
    if start < 1 or end > chrom.length:
        sys.stderr.write(
            "WARNING: skipping read aligned past end of "
    ...
    if read.qlen != read.rlen:
        sys.stderr.write(
            "WARNING skipping read: handling of "
    ...
    snp_idx, read_offset, is_split, overlap_indel = choose_overlap_snp(
        read, snp_tab, snp_index_array, hap_tab, ind_idx
    )
    ...
    if read_count_array[start - 1] < max_count:
        ...
        if not start in warned_pos:
            sys.stderr.write(
                "WARNING read count at position %d "
                "exceeds max %d\n" % (start, max_count)
            )
    ...
    if base == allele1:
        ...
        if ref_array[snp_pos - 1] < max_count:
            ...
            sys.stderr.write(
                "WARNING ref allele count at position %d "
                "exceeds max %d\n" % (snp_pos, max_count)
            )
        ...
        if alt_array[snp_pos - 1] < max_count:
            ...
            sys.stderr.write(
                "WARNING alt allele count at position %d "
                "exceeds max %d\n" % (snp_pos, max_count)
            )
            warned_pos[snp_pos] = True
    else:
        ...
        if other_array[snp_pos - 1] < max_count:
            ...
            sys.stderr.write(
                "WARNING other allele count at position %d "
                "exceeds max %d\n" % (snp_pos, max_count)
            )
...
def write_txt_file(
    out_file, chrom, snp_tab, hap_tab, ind_idx, ref_array, alt_array, other_array
):
    i = 0
...
def main():
    ...
    for chrom in chrom_list:
        ...
        if chrom.name not in snp_tab_h5.root:
            ...
            sys.stderr.write(
                "skipping %s because chromosome with this name "
        ...
        if hap_h5:
            ...
            if len(ind_idx) == 1:
                ind_idx = ind_idx[0]
                sys.stderr.write(
                    "index for individual %s is %d\n" % (args.individual, ind_idx)
                )
            else:
                raise ValueError(
                    "got sample indices for %d individuals, "
                    "but expected to get index for one "
                    "individual (%s)" % (len(ind_idx), args.individual)
                )
                hap_tab = None
                ind_idx = None
        else:
