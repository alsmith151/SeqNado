from seqnado.utils import check_options
...
rule trimgalore_paired:
    ...
    threads: 4
    params:
        options = check_options(config['trim_galore'].get('options', ''))
    log:
        "logs/trimming/{sample}.log",
    shell:
        """
        trim_galore --cores {threads} --trim-n --paired --output_dir trimmed {input.fq1} {input.fq2} {params.options} >> {log} 2>&1 &&
...
rule trimgalore_single:
    ...
    log:
        "logs/trimming/{sample}.log",
    shell:
        """trim_galore --cores {threads} --trim-n --output_dir trimmed {input.fq1} {params.options} > {log} 2>&1 &&
