use rule align_paired as align_paired_spikein with:
    ...
    benchmark: "seqnado_output/benchmark/aligned_spikein/{sample}_align.benchmark" if config.get("benchmark", False) else None,
...
use rule sort_bam as sort_bam_spikein with:
    ...
    benchmark: "seqnado_output/benchmark/aligned_spikein/{sample}_sort.benchmark" if config.get("benchmark", False) else None,
...
use rule index_bam as index_bam_spikein with:
    ...
    benchmark: "seqnado_output/benchmark/aligned_spikein/{sample}_index.benchmark" if config.get("benchmark", False) else None,
...
rule filter_bam_spikein:
    ...
    benchmark: "seqnado_output/benchmark/aligned_spikein/{sample}_filter.benchmark" if config.get("benchmark", False) else None,
    shell:
        """
...
use rule index_bam as index_bam_spikein_filtered with:
    ...
    benchmark: "seqnado_output/benchmark/aligned_spikein/{sample}_filter_index.benchmark" if config.get("benchmark", False) else None,
...
rule split_bam:
    ...
    benchmark: "seqnado_output/benchmark/split_bam/{sample}.benchmark" if config.get("benchmark", False) else None,
    shell:
        """
...
rule move_ref_bam:
    ...
    benchmark: "seqnado_output/benchmark/move_ref_bam/{sample}.benchmark" if config.get("benchmark", False) else None,
    shell:
        """
...
if config.get("spikein_options", {}).get("normalisation_method") == "orlando":
    ...
    rule calculate_normalisation_factors:
        ...
        benchmark: "seqnado_output/benchmark/normalisation_factors_{group}.benchmark" if config.get("benchmark", False) else None,
        script:
