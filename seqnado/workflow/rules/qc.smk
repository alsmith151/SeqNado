...
rule fastqc_raw_paired:
    ...
    benchmark: repeat("seqnado_output/benchmark/fastqc_raw/{sample}.benchmark", 3) if config.get("benchmark", False) else None,
    shell:
        """
...
rule qualimap_bamqc:
    ...
    benchmark: repeat("seqnado_output/benchmark/qualimap_bamqc/{sample}.benchmark", 3) if config.get("benchmark", False) else None,
    shell:"""
...
rule qualimap_rnaseq:
    ...
    benchmark: repeat("seqnado_output/benchmark/qualimap_rnaseq/{sample}.benchmark", 3) if config.get("benchmark", False) else None,
    shell:"""
...
rule frip_enrichment:
    ...
    benchmark: repeat("seqnado_output/benchmark/frip_enrichment/{directory}/{sample}.benchmark", 3) if config.get("benchmark", False) else None,
    shell:
        """
...
rule seqnado_report:
    ...
    benchmark: repeat("seqnado_output/benchmark/seqnado_report.benchmark", 3) if config.get("benchmark", False) else None,
    resources:
        mem=lambda wildcards, attempt: define_memory_requested(initial_value=2, attempts=attempt, scale=SCALE_RESOURCES),
        runtime=lambda wildcards, attempt: define_time_requested(initial_value=1, attempts=attempt, scale=SCALE_RESOURCES),
    container: "library://cchahrou/seqnado/seqnado_qc.sif:latest"
