...
rule fastqc_raw_paired:
    ...
    benchmark: "seqnado_output/benchmark/fastqc_raw/{sample}.benchmark" if config.get("benchmark", False) else None,
    shell:
        """
...
rule qualimap_bamqc:
    ...
    benchmark: "seqnado_output/benchmark/qualimap_bamqc/{sample}.benchmark" if config.get("benchmark", False) else None,
    shell:"""
...
rule qualimap_rnaseq:
    ...
    benchmark: "seqnado_output/benchmark/qualimap_rnaseq/{sample}.benchmark" if config.get("benchmark", False) else None,
    shell:"""
...
rule frip_enrichment:
    ...
    benchmark: "seqnado_output/benchmark/frip_enrichment/{directory}/{sample}.benchmark" if config.get("benchmark", False) else None,
    shell:
        """
...
rule seqnado_report:
    ...
    benchmark: "seqnado_output/benchmark/seqnado_report.benchmark" if config.get("benchmark", False) else None,
    resources:
        mem=lambda wildcards, attempt: define_memory_requested(initial_value=2, attempts=attempt, scale=SCALE_RESOURCES),
        runtime=lambda wildcards, attempt: define_time_requested(initial_value=1, attempts=attempt, scale=SCALE_RESOURCES),
    container: "library://cchahrou/seqnado/seqnado_qc.sif:latest"
