...
rule sort_bam:
    ...
    benchmark: ".benchmarks/alignment_post_process/{sample}_sort.benchmark",
    shell:"""
...
rule index_bam:
    ...
    benchmark: ".benchmarks/alignment_post_process/{sample}_index.benchmark",
...
if config["remove_blacklist"] and os.path.exists(config.get("blacklist", "")):
    ...
    rule remove_blacklisted_regions:
        ...
        benchmark: ".benchmarks/alignment_post_process/{sample}_blacklist.benchmark",
        shell:"""
...
if config["remove_pcr_duplicates_method"] == "picard":
    rule remove_duplicates_using_picard:
        ...
        benchmark: ".benchmarks/alignment_post_process/{sample}_remove_duplicates.benchmark",
        shell:"""
...
if config.get("shift_atac_reads"):
    rule shift_atac_alignments:
        ...
        benchmark: ".benchmarks/alignment_post_process/{sample}_atac_shift.benchmark",
        shell:"""
...
rule filter_bam:
    ...
    benchmark: ".benchmarks/alignment_post_process/{sample}_filter.benchmark",
    params:
...
rule move_bam_to_final_location:
    ...
    benchmark: ".benchmarks/alignment_post_process/{sample}_final.benchmark",
    shell:"""
    mv {input.bam} {output.bam} &&
    mv {input.bai} {output.bai} &&
...
rule bam_stats:
    ...
    benchmark: ".benchmarks/alignment_post_process/{sample}_alignment_stats.benchmark",
    shell: """
...
rule prepare_stats_report:
    ...
    benchmark: ".benchmarks/alignment_stats.benchmark",
    script:
...
use rule index_bam as index_consensus_bam with:
    ...
    benchmark: ".benchmarks/merge_bam/{group}_index.benchmark",
