...
rule fastq_screen_paired:
    ...
    benchmark: repeat("seqnado_output/benchmark/fastq_screen/{sample}_{read}.txt", 3) if config.get("benchmark", False) else None,
    shell:
...
use rule fastq_screen_paired as fastq_screen_single with:
    ...
    benchmark: repeat("seqnado_output/benchmark/fastq_screen/{sample}.txt", 3) if config.get("benchmark", False) else None,
