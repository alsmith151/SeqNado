...
rule heatmap_matrix:
    ...
    params: 
        options=seqnado.utils.check_options(config['heatmap']['options']),
...
rule heatmap_plot:
    ...
    shell: """plotHeatmap --colorMap {params.colormap} \
    --boxAroundHeatmaps no -m {input.matrix} -out {output.heatmap} >> {log} 2>&1"""
