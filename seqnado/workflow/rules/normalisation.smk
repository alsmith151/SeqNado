...
rule calculate_normalisation_factors:
    input:
        bam_ref = expand(bam=rules.move_bam_to_final_location.output.bam),
        bam_spikein = expand(bam=rules.filter_bam_spikein.output.split_bam),
        bam_spikein_index =expand(bam=rules.move_bam_to_final_location.output.bai),
