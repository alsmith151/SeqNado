...
rule make_dataset_regions:
    ...
    benchmark: repeat("seqnado_output/benchmark/make_dataset_regions.txt", 3) if config.get("benchmark", False) else None,
    shell:"""
...
rule make_dataset_binsize:
    ...
    benchmark: repeat("seqnado_output/benchmark/make_dataset_binsize.txt", 3) if config.get("benchmark", False) else None,
    shell:"""
    quantnado-make-dataset \
    --bigwig-dir {params.bigwig_dir} \
    --output-file {output.dataset} \
