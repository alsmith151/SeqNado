...
def generate_fastq_screen_config(
    ...
    for genome_name, genome_config in genome_configs.items():
        ...
        if "_" in genome_name and any(x in genome_name for x in ["dm6", "mm10", "mm39", "ecoli"]):
            continue
    ...
    for key, (genome_name, genome_config) in sorted(genome_groups.items()):
        ...
        if index_path:
            ...
            if parent.exists():
                ...
                index_files = list(parent.glob(f"{prefix}*.bt2")) + list(parent.glob(f"{prefix}*.bt2l"))
                if index_files:
                    lines.append("#" + "-" * 31)
                    ...
                    print(f"⚠️  Warning: No bowtie2 index files found for {genome_name} at {index_path}")
            else:
                print(f"⚠️  Warning: Index path does not exist for {genome_name}: {parent}")
    
    # Add common contaminants if requested
    if include_contaminants:
        if contaminant_base_path is None:
            print("⚠️  Warning: include_contaminants=True but no contaminant_base_path provided")
            print("    Skipping contaminant databases. Use --contaminant-path to specify location.")
        ...
        for name, rel_path in contaminants:
            ...
            print(f"⚠️  Warning: Contaminant path does not exist: {contaminant_base_path}")
...
def load_genome_configs_for_fastqscreen() -> Dict[str, GenomeConfig]:
    ...
    if not config_path.exists():
        raise FileNotFoundError(
            f"Genome config not found at {config_path}. "
            "Run 'seqnado init' first."
        )
