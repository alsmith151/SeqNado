    ...
    for name, config_dict in genome_data.items(): 
        ...
        try:
            genome_configs[name] = GenomeConfig(
                ...
                genes=pathlib.Path(config_dict["genes"]) if config_dict.get("genes") else None,
                blacklist=pathlib.Path(config_dict["blacklist"]) if config_dict.get("blacklist") else None,
                organism=config_dict.get("organism"),
                version=config_dict.get("version"),
            )
        except ValidationError as e:
            for error in e.errors():
                if error.get("type") == "value_error" and hasattr(error.get("ctx", {}), "get"):
...
def get_project_config() -> ProjectConfig:
    ...
    return ProjectConfig(
        name=project_name,
        date=today,
        directory=pathlib.Path(project_dir)
    )
...
def select_genome_config(genome_configs: Dict[str, GenomeConfig]) -> GenomeConfig:
    ...
    while genome_name not in genome_configs:
        print(f"Genome '{genome_name}' is not configured. Please choose from: {', '.join(available_genomes)}")
        genome_name = get_user_input("Genome?", default=available_genomes[0] if available_genomes else "")
...
def get_bigwig_config() -> Optional[BigwigConfig]:
    ...
    return BigwigConfig(
        pileup_method=[PileupMethod(pileup_method)],
        binsize=binsize
    )
...
def get_plotting_config() -> Optional[PlottingConfig]:
    ...
    perform_plotting = get_user_input("Perform plotting?", default="no", is_boolean=True)
...
def get_ucsc_hub_config() -> Optional[UCSCHubConfig]:
    ...
    color_by = get_user_input(
        "Color by (for UCSC hub):",
        default="samplename"
    )
...
def get_rna_quantification_config() -> Optional[RNAQuantificationConfig]:
    ...
    method = get_user_input(
        ...
        choices=[m.value for m in RNAQuantificationMethod],
    ...
    return RNAQuantificationConfig(
        method=RNAQuantificationMethod(method),
...
def get_mcc_config() -> Optional[MCCConfig]:
    ...
    viewpoints = get_user_input(
        ...
        default="path/to/viewpoints.bed",
        is_path=True
    )
    
    resolutions_str = get_user_input(
        "Resolutions for MCC cooler files (comma-separated):",
        default="100,1000"
    )
    ...
    return MCCConfig(
        viewpoints=pathlib.Path(viewpoints),
...
def get_methylation_config() -> Optional[MethylationConfig]:
    ...
    call_methylation = get_user_input("Call methylation?", default="no", is_boolean=True)
...
def build_assay_config(assay: Assay, genome_config: GenomeConfig) -> Optional[AssaySpecificConfig]:
