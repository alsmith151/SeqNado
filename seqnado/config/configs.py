...
class GenomeIndex(Enum):
    """Enum for different types of genome indices."""
...
class GenomeConfig(BaseModel):
    ...
    def validate_paths_exist(cls, v: Path | None) -> Path | None:
        if v is not None and not v.exists():
            ...
            error_msg = f"❌ Genome file not found\n\n"
            ...
            if parent_dir.exists():
                ...
                try:
                    ...
                    if all_files:
                        ...
                        similar_files = [f for f in all_files if filename_lower in f.lower() or f.lower() in filename_lower]
                        ...
                        if other_files and len(all_files) <= 15:
                            ...
                            if len(other_files) > 10:
                                error_msg += f"   ... and {len(other_files) - 10} more files\n"
                        elif other_files:
...
class QC(BaseModel):
    ...
    fraction_of_reads_in_peaks: bool = True
...
class ComputedFieldsMixin:
    ...
    @computed_field
    @property
    def make_bigwigs(self) -> bool:
        """Whether to make bigwigs (computed from bigwigs config presence)."""
        return getattr(self, 'bigwigs', None) is not None

    @computed_field
    @property
    def make_heatmaps(self) -> bool:
        """Whether to make heatmaps."""
        return getattr(self, 'heatmaps', False)

    @computed_field
    @property
    def perform_plotting(self) -> bool:
...
class PathValidatorMixin:
    ...
    @staticmethod
    def validate_path_exists(v: Path | str | None, field_name: str = "path") -> Path | str | None:
    ...
    @staticmethod
    def validate_required_when(v: str | None, condition: bool, field_name: str) -> str | None:
...
class BaseAssayConfig(BaseModel, ComputedFieldsMixin):
    ...
    # Tool options as raw YAML string
    options: str = ""
...
class WorkflowConfig(BaseModel):
    ...
    pcr_duplicates: PCRDuplicates = PCRDuplicates()
    qc: QC = QC()
